...
#### Polling 
At 10V, I measured the encoder output frequency using an oscilloscope and found it to be around 1 kHz. This means the motor speed is 
1000/408 = 2.45 rev/sec. Then, I wrote code that toggled a pin inside a while loop in main to test how fast the MCU could perform polling. 
I measured the frequency of this signal and found it to be 549 Hz, which corresponds to about 1.35 rev/sec. This shows that the polling 
rate is slower than the encoder frequency, so the MCU would miss some edges since it is not able to sample as fast. Therefore, interrupts are 
needed to correctly capture all the encoder transitions.
